...
if(me.args())
    Std.system("echo New Pitch Manipulation " + me.arg(0) + ">> log.txt");
else
    Std.system("echo New Pitch Manipulation >> log.txt");
...
12      => int target_pitch_diff;
target_pitch_diff * input_data_range / 32. + input_data_median + 0.5
    => Std.ftoi => int target_input;
// Keyboard input 
...
fun float evaluate_pitch(int scale)
{
    serial_input => int now_serial;
    (now_serial - input_data_median) * 32 / input_data_range => int now_pitch;

    if((now_serial - latest_serial) * 32 / input_data_range != 0)
    {
        Std.system("echo Pitch: " + Std.itoa(now_pitch) + " >> log.txt");
        chout <= "Pitch: " <= now_pitch <= "\n";
        now_serial => latest_serial;
    }
    return Std.mtof(scale +
        (now_serial - input_data_median) * 32 / input_data_range);
}
...
chout <= "Pitch manipulation session begins.\n";
...
while(true)
{
    ...
    {
        scale_list[i] => evaluate_pitch => instrument.freq;
